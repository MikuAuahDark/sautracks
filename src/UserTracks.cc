...
char *myDocumentsPath = nullptr;
char *uTraxPath = nullptr;
char *uFilesPath = nullptr;

typedef struct UTrax
{
	int startIndex;
	int stringLength;
	int audioType;
} UTrax;
static_assert(sizeof(UTrax) == 12, "invalid struct size");

void initMyDocumentsPath()
{
...
errno_t UserTracksSave(std::vector<const char*> fileList)
{
	...
	for (auto i = fileList.begin(); i != fileList.end(); i++)
	{
		...
		// Detect audio type by extension
		{
			...
			if (!strcmpi(ext, ".ogg") || !strcmpi(ext, ".oga")) extid = 1; // assume vorbis
			else if (!strcmpi(ext, ".wav")) extid = 2; // assume PCM
			else if (!strcmpi(ext, ".mp3")) extid = 3;
			else if (!strcmpi(ext, ".aac") || !strcmpi(ext, ".m4a")) extid = 4; // assume AAC
		}
